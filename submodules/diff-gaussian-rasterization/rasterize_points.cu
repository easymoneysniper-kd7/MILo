	...
	const bool debug)
{
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		out_coord.contiguous().data_ptr<float>(),
		out_mcoord.contiguous().data_ptr<float>(),
	...
	const bool debug) 
{
  ...
  {  
	  ...
	  dL_dout_coord.contiguous().data_ptr<float>(),
	  dL_dout_mcoord.contiguous().data_ptr<float>(),
	  ...
	  dL_dview_points.contiguous().data_ptr<float>(),
	  dL_dconic.contiguous().data_ptr<float>(),  
	  dL_dopacity.contiguous().data_ptr<float>(),
	  dL_dcolors.contiguous().data_ptr<float>(),
	  dL_dts.contiguous().data_ptr<float>(),
	  dL_dcamera_planes.contiguous().data_ptr<float>(),
	  dL_dray_planes.contiguous().data_ptr<float>(),
